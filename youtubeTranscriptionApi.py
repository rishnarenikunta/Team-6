...
def try_subtitles_first(video_id: str) -> Optional[str]:
    ...
    #                 "subtitlesformat": "vtt",
    #                 "outtmpl": os.path.join(tmpdir, "%(id)s.%(ext)s"),
    #                 "ignore_no_formats_error": True,  # don't fail if video format unavailable
    #             }
    #             with ydl_with_cookies(opts) as ydl:
    #                 ydl.download([video_url])

    #             vtts = [os.path.join(tmpdir, f) for f in os.listdir(tmpdir) if f.endswith(".vtt")]
    #             print(f"[SUBTITLE FILES attempt {attempt}] {video_id} found={len(vtts)}")
    ...
    #     if attempt < 3:
    #         print(f"[SUBTITLE RETRY] {video_id} retrying...")
    #         time.sleep(2)
    ...
    for bucket, channels in CHANNEL_BUCKETS.items():
        ...
        for channel_url in channels:
            ...
            try:
                ...
                print(f"[CHANNEL FAIL] {channel_url} err={str(e)[:300]}")
                results[bucket].append({
                    "channel_url": channel_url,
                    "weights": {"views": w_views, "likes": w_likes, "comments": w_comments},
                    "top_20": [],
                    "stored_count": 0,
                    "target_count": TARGET_PER_CHANNEL,
                })
                continue
            ...
            for rank, v in enumerate(scored, start=1):
                if len(stored) >= TARGET_PER_CHANNEL:
                    break

                vid = v["video_id"]
                if not vid or vid in stored_ids:
                    continue

                print(f"[STORE TRY {len(stored)+1}/{TARGET_PER_CHANNEL}] rank={rank} vid={vid}")
                ...
                try:
                    ...
                    metadata_ok = True
                    print(f"[METADATA OK] {vid}")
                except Exception as e:
                    print(f"[METADATA SKIP] {vid} err={str(e)[:200]}")
                    continue

                try:
                    get_comments(vid)
                    print(f"[COMMENTS OK] {vid}")
                except Exception as e:
                    print(f"[COMMENTS FAIL] {vid} err={str(e)[:200]}")
                ...
                print(f"[STORE OK] {vid} total_stored={len(stored)}")
                time.sleep(2)
